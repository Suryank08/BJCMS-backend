...
public class RandomPasswordGenerator {
    ...
    public static String generatePassword(int length, String fName, String mobileNum, String email) {
        StringBuilder password = new StringBuilder(length);

        // Use part of first name or last name
        if (fName.length() > 2) {
            password.append(fName.substring(0, 2)); // Take first 2 letters of the first name
        }

        // Add some digits from the mobile number
        if (mobileNum.length() >= 2) {
            password.append(mobileNum.substring(0, 2)); // Take first 2 digits from mobile number
        }
